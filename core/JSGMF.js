...
window.game_background = function(){return 'white';}
window.game_wbackground = function(){return 'black';}
window.game_title = function(){return 'NEW GAME';}
window.game_width = function(){return 1000;}
window.game_height = function(){return 1000;}
window.game_padding = function(){return 0;}

window.game_draw = function() {
	console.warn("NO DRAW OVER-RIDE");
};

window.game_init = function() {
	console.warn("NO INIT OVER-RIDE");
};

window.JSGMF = {}

//INIT SUPPRESSED KEYS
JSGMF.initSuppressed = function() {
	this.suppressed = this.suppressed || [];
}

JSGMF.suppress = function(x) {
	JSGMF.initSuppressed()
	if (this.suppressed.indexOf(x) == -1)
		this.suppressed.push(x);
}

JSGMF.isSuppressed = function(x) {
	JSGMF.initSuppressed()
	return (this.suppressed.indexOf(x) != -1)
}

//AUDIO
JSGMF.initAudio = function() {
	this.audio = [];
}

JSGMF.addAudio = function(name, srcd, local) {
...
JSGMF.set = function() {
	function engine_draw()
	{
		...
		ctx.scale(game_scale[0], game_scale[1])
...
JSGMF.press = function(x) {
	var c = x.charCodeAt(0)-32;
	window.game.keys[c] = 2;
}

JSGMF.unPress = function(x) {
	var c = x.charCodeAt(0)-32;
	window.game.keys[c] = 0;
}

JSGMF.getKeyPressed = function(x) {
	if (typeof x == typeof "")
		var c = x.charCodeAt(0)-32;
	else
		var c = +x;
...
JSGMF.init = function(useCanvas) {
	...
	{
		if (e.button == 0 && document.mouse.left == 0){document.mouse.left = 2;if (JSGMF.isSuppressed("lmb")) {e.preventDefault()}}
		if (e.button == 1 && document.mouse.middle == 0){document.mouse.middle = 2;if (JSGMF.isSuppressed("mmb")) {e.preventDefault()}}
		if (e.button == 2 && document.mouse.right == 0){document.mouse.right = 2;if (JSGMF.isSuppressed("rmb")) {e.preventDefault()}}
	}

	document.oncontextmenu = function(e)
	{
		if (JSGMF.isSuppressed("context"))
			e.stopPropagation();
			e.preventDefault();
	}
...
//Extra Definitions for CTX prototype
